-- Drop existing voting trigger if it exists
DROP TRIGGER IF EXISTS update_story_vote_counts ON public.story_votes;
DROP TRIGGER IF EXISTS update_story_vote_counts_only ON public.story_votes;

-- Drop existing function if it exists
DROP FUNCTION IF EXISTS update_story_vote_counts_only();

-- Create a new function to handle vote count updates
